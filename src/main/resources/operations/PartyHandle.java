...
public class PartyHandle {
     ...
     * @return
     */
    public Operation party_set_morale(int morale){

        return new PartySetMorale(this.variable,Variable.number(Math.min(Math.max(morale,0),100)));
    }

    public Operation party_upgrade_with_xp(Variable xpAmount, Commons.UpgradePath upgradePath){
        return new PartyUpgradeWithXp(this.variable,xpAmount,variable.number(upgradePath.getValue()));
    }

    public Operation party_add_xp(Variable xpAmount){
        return new PartyAddXp(this.variable,xpAmount);
    }

    public Operation party_add_template(PartyTemplate partyTemplate,boolean isPrisoner){
        return new PartyAddTemplate(this.variable,partyTemplate.getVar(),Variable.number(isPrisoner ? 1 : 0));
    }

    public Operation party_set_icon(Variable mapIcon){
        return new PartySetIcon(this.variable,mapIcon);
    }

    public Operation party_set_banner_icon(Variable mapIcon){
        return new PartySetBannerIcon(this.variable,mapIcon);
    }

    public Operation party_add_particle_system(Variable particleSystem){
        return new PartyAddParticleSystem(this.variable,particleSystem);
    }

    public Operation party_clear_particle_systems(){
        return new PartyClearParticleSystems(this.variable);
    }

    public Operation party_get_battle_opponent(Variable destination){
